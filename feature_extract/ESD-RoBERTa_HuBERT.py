...
import pickle
import pandas as pd
from transformers import RobertaTokenizer, AutoTokenizer, RobertaModel, Wav2Vec2Processor, HubertModel
...
def process_row(row, tokenizers, text_models, hubert_processor, hubert_model, device):
...
def process_dataset(input_path, output_path, tokenizers, text_models, hubert_processor, hubert_model, device):
        ...
        for _, row in tqdm(data_list.iterrows(), total=len(data_list), desc=f"Processing {output_path.split('/')[-1]}"):
            if row['language'] in tokenizers:
                processed_data.append(
                    process_row(row, tokenizers, text_models, hubert_processor, hubert_model, device)
                )
...
def main():
    ...
    process_dataset(
        ESD_TRAIN_PATH,
        f"{OUTPUT_DIR}ESD_ENG_CMN_RoBERTa_HuBERT_train.pkl",
        tokenizers,
        text_models,
        HUBERT_PROCESSOR,
        HUBERT_MODEL,
    ...
    process_dataset(
        ESD_VAL_PATH,
        f"{OUTPUT_DIR}ESD_ENG_CMN_RoBERTa_HuBERT_val.pkl",
        tokenizers,
        text_models,
        HUBERT_PROCESSOR,
        HUBERT_MODEL,
    ...
    process_dataset(
        ESD_TEST_PATH,
        f"{OUTPUT_DIR}ESD_ENG_CMN_RoBERTa_HuBERT_test.pkl",
        tokenizers,
        text_models,
        HUBERT_PROCESSOR,
        HUBERT_MODEL,
