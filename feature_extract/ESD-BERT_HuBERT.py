...
import pickle
import pandas as pd
from transformers import BertTokenizer, BertModel, Wav2Vec2Processor, HubertModel
...
def process_row(row, tokenizer_eng, text_model_eng, tokenizer_cmn, text_model_cmn, hubert_processor, hubert_model, device):
    # Process text
    text = row['raw_text']
...
def process_dataset(input_path, output_path, tokenizer_eng, text_model_eng, tokenizer_cmn, text_model_cmn, hubert_processor, hubert_model, device):
    ...
    with torch.no_grad():
        for _, row in tqdm(data_list.iterrows(), total=len(data_list), desc=f"Processing {output_path.split('/')[-1]}"):
            processed_data.append(
                process_row(row, tokenizer_eng, text_model_eng, tokenizer_cmn, text_model_cmn, hubert_processor, hubert_model, device)
            )
...
def main():
    ...
    process_dataset(
        ESD_TRAIN_PATH,
        f"{OUTPUT_DIR}ESD_ENG_CMN_BERT_HuBERT_train.pkl",
        TOKENIZER_ENG,
        TEXT_MODEL_ENG,
        TOKENIZER_CMN,
        TEXT_MODEL_CMN,
    ...
    process_dataset(
        ESD_VAL_PATH,
        f"{OUTPUT_DIR}ESD_ENG_CMN_BERT_HuBERT_val.pkl",
        TOKENIZER_ENG,
        TEXT_MODEL_ENG,
        TOKENIZER_CMN,
        TEXT_MODEL_CMN,
    ...
    process_dataset(
        ESD_TEST_PATH,
        f"{OUTPUT_DIR}ESD_ENG_CMN_BERT_HuBERT_test.pkl",
        TOKENIZER_ENG,
        TEXT_MODEL_ENG,
        TOKENIZER_CMN,
        TEXT_MODEL_CMN,
