...
checkpoint = torch.load('/home/nhut-minh-nguyen/Documents/FuzzyFusion-SER/FlexibleMMSER/fine-tuning/model/best_hubert_embeddings.pt')
HUBERT_MODEL.load_state_dict(checkpoint['model_state_dict'])
HUBERT_MODEL.eval()

def extract_hubert_features(audio_file, wav2vec_processor, hubert_model, device):
    ...
    with torch.no_grad():
        embeddings, projections = hubert_model(input_values)
...
def process_row(row, tokenizer, text_model, wav2vec_processor, hubert_model, device):
    ...
    audio_embed = extract_hubert_features(
        audio_file, 
        wav2vec_processor, 
        hubert_model, 
        device
...
def main():
    ...
    process_dataset(
        IEMOCAP_TRAIN_PATH,
        f"{OUTPUT_DIR}IEMOCAP_BERT_HUBERT_train.pkl",
        TOKENIZER,
        TEXT_MODEL,
        WAV2VEC_PROCESSOR,
        HUBERT_MODEL,
    ...
    process_dataset(
        IEMOCAP_VAL_PATH,
        f"{OUTPUT_DIR}IEMOCAP_BERT_HUBERT_val.pkl",
        TOKENIZER,
        TEXT_MODEL,
        WAV2VEC_PROCESSOR,
        HUBERT_MODEL,
    ...
    process_dataset(
        IEMOCAP_TEST_PATH,
        f"{OUTPUT_DIR}IEMOCAP_BERT_HUBERT_test.pkl",
        TOKENIZER,
        TEXT_MODEL,
        WAV2VEC_PROCESSOR,
        HUBERT_MODEL,
