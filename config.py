...
def model_prediction(model, dataloader, accuracy_fn):
    eval_wa, eval_ua = 0.0, 0.0
    y_true_ls, y_pred_ls = [], []

    model.eval()
    with torch.no_grad():
        for batch, (text_embed, audio_embed, labels) in enumerate(dataloader):
            ...
            y_preds = output_softmax.argmax(dim=1)

            wa, ua = accuracy_fn(y_preds.cpu().numpy(), labels.cpu().numpy())
            eval_wa += wa
            eval_ua += ua

            y_true_ls.extend(labels.cpu().numpy())
            y_pred_ls.extend(y_preds.cpu().numpy())
